...
export function* getBranchEmployees(id) {
  const accessToken = yield select(AppSelectors.makeSelectAccessToken());
  const user = yield select(AppSelectors.makeSelectCurrentUser());
  const requestURL = `${Endpoints.GetBranchEmployeesApi}/${id}/${user &&
...
export function* getDeptEmployees(id) {
  const accessToken = yield select(AppSelectors.makeSelectAccessToken());
  const user = yield select(AppSelectors.makeSelectCurrentUser());
  const requestURL = `${Endpoints.GetDeptEmployeesApi}/${id}/${user &&
    user.organisation.id}`;
...
export function* getPartyGroups() {
  const accessToken = yield select(AppSelectors.makeSelectAccessToken());
  const user = yield select(AppSelectors.makeSelectCurrentUser());
  const requestURL = `${Endpoints.GetPartyGroups}?orgId=${user &&
...
export function* getEnrollmentTypes() {
  const accessToken = yield select(AppSelectors.makeSelectAccessToken());
  const user = yield select(AppSelectors.makeSelectCurrentUser());
  const requestURL = `${Endpoints.GetEnrollmentTypes}?orgId=${user &&
    user.organisation.orgId}&type=ENROLLMENTTYPE`;
...
export function* getLocations() {
  const accessToken = yield select(AppSelectors.makeSelectAccessToken());
  const user = yield select(AppSelectors.makeSelectCurrentUser());
  const requestURL = `${Endpoints.GetLocations}?orgId=${user &&
    user.organisation.orgId}&type=LOCATION`;
...
export function* getJobApplications({ payload }) {
  const accessToken = yield select(AppSelectors.makeSelectAccessToken());
  const user = yield select(AppSelectors.makeSelectCurrentUser());
  const requestURL = `${Endpoints.GetJobApplications}/${user &&
    user.organisation.orgId}`;
...
export function* getJobOpenings() {
  const accessToken = yield select(AppSelectors.makeSelectAccessToken());
  const user = yield select(AppSelectors.makeSelectCurrentUser());
  const requestURL = `${Endpoints.GetJobOpenings}/${user &&
    user.organisation.orgId}`;
...
export function* getRoles() {
  const accessToken = yield select(AppSelectors.makeSelectAccessToken());
  const user = yield select(AppSelectors.makeSelectCurrentUser());
  const requestURL = `${Endpoints.GetRoles}?orgId=${user &&
    user.organisation.orgId}&type=ROLE`;
...
export function* getAnnouncements() {
  const accessToken = yield select(AppSelectors.makeSelectAccessToken());
  const user = yield select(AppSelectors.makeSelectCurrentUser());
  const requestURL = `${Endpoints.GetAnnouncements}/${user &&
    user.organisation.orgId}?start=1&limit=5`;
...
export function* getAttendances() {
  const accessToken = yield select(AppSelectors.makeSelectAccessToken());
  const user = yield select(AppSelectors.makeSelectCurrentUser());
  const requestURL = `${Endpoints.GetAttendances}/${user &&
    user.organisation.orgId}`;
...
export function* createBranch({ type, payload }) {
  ...
  try {
    ...
    yield put(
      AppActions.openSnackBar({
        message: `${response.name} Branch created`,
        status: 'success',
      }),
    );
...
export function* deleteAnnouncement({ payload }) {
  const accessToken = yield select(AppSelectors.makeSelectAccessToken());
  const requestURL = `${Endpoints.DeleteAnnouncementApi}?announcementId=${
    payload.id
  }`;
...
export default function* HRRootSaga() {
  ...
  yield takeLatest(
    Constants.GET_DEPARTMENTS_BY_ORGID_API,
  ...
  yield takeLatest(
    Constants.CREATE_EDUCATION_BACKGROUND,
